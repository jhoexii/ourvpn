#!/bin/bash
apt-get update && apt-get upgrade -y && update-grub && sleep 2 >/dev/null 2>&1
if [ "${EUID}" -ne 0 ]; then
...
#!/bin/bash
wget -O /usr/local/sbin/jho/active.sh http://slowdnsapi.gigalife.cf/ourvpn/app/prem
wget -O /usr/local/sbin/jho/inactive.sh http://slowdnsapi.gigalife.cf/ourvpn/app/xprem
wget -O /usr/local/sbin/jho/deleted.sh http://slowdnsapi.gigalife.cf/ourvpn/app/deleted
